...
class MBDynHelper:
    ...
    def initialize(self, case='shell'):
        ...
        self.input_file_name = self.main_path + '/../mbdynData/' + case + '.mbd'
        ...
        path = self.main_path + '/../mbdynData/{name}.sock'.format(name=case)
        '''if len(path) > 107:
        	exit("\n\nPlease use a shorter MBDyn socket path! (Limit: 107 characters)\nCurrent path: " + path + "\nMain path:" + self.main_path + "\n\n")'''
        path = "mbdyn/mbdynData/{name}.sock".format(name=case)
